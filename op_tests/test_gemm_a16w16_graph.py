...
# The value rocgdb read out of the wedged counter on the hung endpoint.
_STALE = 2
...
def _run_child() -> None:
    ...
    # Eager reference first: it must run against a clean counter, and it primes
    # the capture ring so the staleness below can reach every slot.
    ...
    # Seed the residue rather than racing for it. Every buffer a captured launch
    # could be handed starts dirty, so only a zero-fill recorded inside the
    # graph can re-establish the counter == 0 entry state on replay.
    capture_stream = torch.cuda.Stream(device=device)
    with torch.cuda.stream(capture_stream):
        get_semaphore_workspace(device).fill_(_STALE)
        for slot in _get_capture_ring(device.index):
            slot.fill_(_STALE)
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires a ROCm GPU")
def test_capture_ring_is_bounded_and_rotates() -> None:
    """Captured launches must not alias until the fixed ring is exhausted."""
    from aiter.ops.gemm_op_a16w16 import (
        _CAPTURE_RING_SIZE,
        _get_capture_ring,
        _next_captured_workspace,
    )

    device_index = torch.cuda.current_device()
    ring = _get_capture_ring(device_index)
    assert len(ring) == _CAPTURE_RING_SIZE
    assert len({slot.data_ptr() for slot in ring}) == _CAPTURE_RING_SIZE

    handed_out = [
        _next_captured_workspace(device_index).data_ptr()
        for _ in range(_CAPTURE_RING_SIZE)
    ]
    # A full sweep of the ring hands out every slot exactly once.
    assert set(handed_out) == {slot.data_ptr() for slot in ring}
